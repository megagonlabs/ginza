...
def evaluate_from_file(
    ...
    system = load_conllu(conllu_file)
...
def load_conllu(conllu_file):
    with open(conllu_file, "r", encoding="utf8") as fin:
        sentences = {}
        sentence = None
        tokens = []
        idx = 0
        for line in fin:
            line = line.rstrip()
            if not line:
                assert sentence and tokens
                for t in tokens:
                    if t.dep_ == "root":
                        t.dep_ = "ROOT"
                        t.head = t
                    else:
                        t.head = tokens[t.head]
            ...
            if line.startswith("# text = "):
                assert not sentence and not tokens
                sentence = line[9:]
                continue
            r = line.split("\t")
            t = TokenFake()
            t.i = int(r[0]) - 1
            t.orth_, t.lemma_, t.pos_, t.tag_ = r[1:5]
            t.head = int(r[6]) - 1
            t.dep_ = r[7]
            t.ent_type_ = None
            t.ent_iob_ = "O"
            if "NE=" in r[9]:
                t.ent_iob_, t.ent_type_ = r[9].split("NE=")[1].split("|")[0].split("-")
            print(sentence, t.orth_)
            t.idx = sentence.index(t.orth_, idx)
            idx = t.idx + len(t.orth_)
            tokens.append(t)
    assert not sentence and not tokens
    return sentences
